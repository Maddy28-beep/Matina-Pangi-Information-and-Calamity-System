...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="mb-0"><i class="bi bi-megaphone"></i> Edit Announcement</h2>
        ...
        <form method="POST" action="{{ route('announcements.update', $announcement) }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    @php $urg = old('urgency', $announcement->urgency ?: ''); @endphp
                    <select name="urgency" class="form-select">
                        <option value="" {{ $urg==='' ? 'selected' : '' }}>Normal</option>
                        <option value="High" {{ $urg==='High' ? 'selected' : '' }}>High</option>
                        <option value="Critical" {{ $urg==='Critical' ? 'selected' : '' }}>Critical</option>
                    </select>
                </div>
                <div class="col-12">
                ...
                <div class="col-12">
                    ...
                    <div class="form-check mb-3 p-3 border rounded">
                        <input class="form-check-input" type="radio" name="target_type" id="entire_barangay" value="all" {{ empty(($announcement->filters['target_purok'] ?? '')) && empty(($announcement->filters['households'] ?? [])) ? 'checked' : '' }}>
                    ...
                    <div class="form-check mb-3 p-3 border rounded">
                        <input class="form-check-input" type="radio" name="target_type" id="specific_purok_radio" value="purok" {{ !empty(($announcement->filters['target_purok'] ?? '')) ? 'checked' : '' }}>
                        ...
                        <div class="mt-2 ms-4" id="purok_selector" style="display:none;">
                            <select name="target_purok" id="target_purok" class="form-select">
                                ...
                                @foreach($puroks as $p)
                                    <option value="{{ $p->purok_name }}" {{ old('target_purok', $announcement->filters['target_purok'] ?? '') == $p->purok_name ? 'selected' : '' }}>{{ $p->purok_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-check mb-3 p-3 border rounded">
                        <input class="form-check-input" type="radio" name="target_type" id="specific_households_radio" value="households" {{ !empty(($announcement->filters['households'] ?? [])) ? 'checked' : '' }}>
                        ...
                        <div class="mt-2 ms-4" id="households_selector" style="display:none;">
                            <input type="text" id="household_search" class="form-control mb-2" placeholder="Search households...">
                            <select name="households[]" id="households_select" class="form-select" multiple size="8">
                                @foreach($households as $hh)
                                    @php $sel = collect(old('households', $announcement->filters['households'] ?? []))->contains($hh->id); @endphp
                                    <option value="{{ $hh->id }}" data-search="{{ strtolower($hh->household_id . ' ' . optional($hh->officialHead)->full_name . ' ' . $hh->purok) }}" {{ $sel ? 'selected' : '' }}>
                    ...
                    <div class="mt-3 p-3 bg-light rounded">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="only_affected" id="only_affected" value="1" {{ old('only_affected', $announcement->filters['only_affected'] ?? false) ? 'checked' : '' }}>
                            <label class="form-check-label" for="only_affected">Only Affected Residents</label>
                        </div>
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" name="only_evacuees" id="only_evacuees" value="1" {{ old('only_evacuees', $announcement->filters['only_evacuees'] ?? false) ? 'checked' : '' }}>
                ...
                <div class="col-12 mt-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="send_sms" id="send_sms" value="1" {{ old('send_sms', $announcement->send_sms) ? 'checked' : '' }}>
                        <label class="form-check-label" for="send_sms">Send SMS</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="send_email" id="send_email" value="1" {{ old('send_email', $announcement->send_email) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function syncVisibility() {
        const sel = document.querySelector('input[name="target_type"]:checked');
        purokSelector.style.display = sel && sel.value === 'purok' ? 'block' : 'none';
        householdsSelector.style.display = sel && sel.value === 'households' ? 'block' : 'none';
    }
    syncVisibility();
    targetRadios.forEach(r => r.addEventListener('change', syncVisibility));

...
<form method="GET" action="{{ route('web.notifications.index') }}" class="card mb-4 filter-inline">
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-center mb-2">
      <h6 class="mb-0"><i class="bi bi-funnel me-2"></i>Filter Notifications</h6>
    </div>
    <div class="border-top mb-3"></div>
    <div class="filter-grid-3">
      <div class="flex-grow-1">
        <label class="form-label small fw-semibold text-uppercase">Title or message</label>
        <input type="text" name="search" class="form-control" placeholder="Title or message content" value="{{ request('search') }}">
      </div>
      <div>
      ...
      <div class="actions d-flex gap-2" style="justify-self:end">
        ...
        <a href="{{ route('web.notifications.index') }}" class="btn btn-filter-outline"><i class="bi bi-x-circle"></i> Clear</a>
      </div>
    </div>
  </div>
</form>
...
<div class="card">
  <div class="card-body">
    ...
    <div class="table-responsive">
      <table class="table table-hover ds-table sortable-table" role="grid" aria-label="Notifications">
        <thead class="table-light">
          <tr>
            <th scope="col" role="columnheader" aria-sort="none">{{ __('Title') }}</th>
            <th scope="col" role="columnheader" aria-sort="none">{{ __('Type') }}</th>
            <th scope="col" role="columnheader" aria-sort="none">{{ __('Status') }}</th>
            <th scope="col" role="columnheader" aria-sort="none">{{ __('Sent At') }}</th>
            <th scope="col" role="columnheader" aria-sort="none">{{ __('Actions') }}</th>
          </tr>
        </thead>
        <tbody>
          @foreach($notifications as $n)
          <tr class="clickable-row" style="cursor: pointer;" data-href="{{ route('web.notifications.show', $n->id) }}">
            ...
            <td onclick="event.stopPropagation()">
              <div class="btn-group btn-group-sm">
                <a href="{{ route('web.notifications.edit', $n->id) }}" class="btn btn-outline-secondary" title="Edit">
                  <i class="bi bi-pencil-fill"></i>
                </a>
                <form action="{{ route('web.notifications.destroy', $n->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Archive this notification?')">
                  @csrf
                  @method('DELETE')

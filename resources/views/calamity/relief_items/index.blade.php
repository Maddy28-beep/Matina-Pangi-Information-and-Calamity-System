...
@section('content')<div class="ds-page">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <div class="d-flex gap-2">
        <a href="{{ route('web.relief-items.create') }}" class="btn btn-primary"><i class="bi bi-plus-circle"></i> Add
          New</a>
      </div>
    </div>
      ...
      <div class="card-body">
        ...
        <div class="filter-grid-3">
          <div class="flex-grow-1">
            ...
            <input type="text" name="search" class="form-control" placeholder="Item name or category"
          ...
          <div>
            ...
            <input type="number" name="min_qty" class="form-control" placeholder="Min Qty"
          ...
          <div class="actions d-flex gap-2" style="justify-self:end">
            ...
            <a href="{{ route('web.relief-items.index') }}" class="btn btn-filter-outline"><i class="bi bi-x-circle"></i>
              Clear</a>
          </div>
        </div>
      </div>
              ...
              <tbody>
                @foreach($items as $item)
                  <tr class="clickable-row" style="cursor: pointer;"
                    data-href="{{ route('web.relief-items.show', $item->id) }}">
                    <td><strong class="text-dark">{{ $item->name }}</strong></td>
                    <td><span class="badge bg-info">{{ ucfirst($item->category) }}</span></td>
                    <td><span class="badge bg-success">{{ $item->quantity }} {{ $item->unit }}</span></td>
                    <td onclick="event.stopPropagation()">
                      <div class="btn-group btn-group-sm">
                        <a href="{{ route('web.relief-items.edit', $item->id) }}" class="btn btn-outline-secondary"
                          title="Edit">
                          <i class="bi bi-pencil-fill"></i>
                        </a>
                        <form action="{{ route('web.relief-items.destroy', $item->id) }}" method="POST" class="d-inline"

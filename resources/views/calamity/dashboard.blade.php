...
@section('title', 'Calamity Dashboard')
...
<div class="dashboard-content-wrapper" style="background: #f6f8f7; min-height: 100vh;">
    <div class="dashboard-content" style="background: #f6f8f7; border-radius: 24px; box-shadow: 0 4px 32px 0 rgba(44, 111, 82, 0.08); padding: 40px 32px; max-width: 1400px; margin: 0 auto;">
        ...
        <div class="stats-grid">
            <div>
                <div class="card stat-card h-100" onclick="window.location.href='{{ route('calamities.index') }}?status=ongoing'">
                    <div class="card-body">
                        <h3 class="fw-bold mb-1 text-danger">{{ number_format($ongoing) }}</h3>
                        <p class="text-muted mb-0">Ongoing Incidents</p>
                    </div>
                </div>
            </div>
            <div>
                <div class="card stat-card h-100" onclick="window.location.href='{{ route('calamities.index') }}?status=resolved'">
                    <div class="card-body">
                        <h3 class="fw-bold mb-1 text-success">{{ number_format($resolved) }}</h3>
                        <p class="text-muted mb-0">Resolved Incidents</p>
                    </div>
                </div>
            </div>
            <div>
                <div class="card stat-card h-100" onclick="window.location.href='{{ route('web.calamity-affected-households.index') }}'">
                    <div class="card-body">
                        <h3 class="fw-bold mb-1">{{ number_format($totalAffectedHouseholds) }}</h3>
                        <p class="text-muted mb-0">Affected Households</p>
                    </div>
                </div>
            </div>
            <div>
                <div class="card stat-card h-100" onclick="window.location.href='{{ route('web.calamity-affected-households.index') }}?needing_relief=1'">
                    <div class="card-body">
                        <h3 class="fw-bold mb-1">{{ number_format($needingRelief) }}</h3>
        ...
        <div class="stats-grid">
            <div>
                <div class="card stat-card h-100" onclick="window.location.href='{{ route('web.response-team-members.index') }}'">
                    <div class="card-body">
                        <h3 class="fw-bold mb-1">{{ number_format($rescuesToday) }}</h3>
                        <p class="text-muted mb-0">Rescues Today</p>
                    </div>
                </div>
            </div>
            <div>
                <div class="card stat-card h-100" onclick="window.location.href='{{ route('web.notifications.index') }}'">
                    <div class="card-body">
                        <h3 class="fw-bold mb-1">{{ number_format($notificationsPending) }}</h3>
                        <p class="text-muted mb-0">Pending Notices</p>
                    </div>
                </div>
            </div>
            <div>
                <div class="card stat-card h-100" onclick="window.location.href='{{ route('calamities.index') }}'">
                    <div class="card-body">
                        <h3 class="fw-bold mb-1">{{ number_format($incidents->total()) }}</h3>
            ...
            <div>
                <div class="card stat-card h-100" onclick="window.location.href='{{ route('calamities.index') }}'">
                    <div class="card-body">
                        <h3 class="fw-bold mb-1">{{ number_format($recentCalamities->count()) }}</h3>
        ...
        <div class="card border-0 shadow-sm mb-5">
            ...
            <div class="card-body">
                <div class="quick-actions-grid">
                    <a class="btn btn-outline-primary w-100" href="{{ route('calamities.index') }}">Manage Incidents</a>
                    <a class="btn btn-outline-primary w-100" href="{{ route('web.calamity-affected-households.index') }}">Affected Households</a>
                    <a class="btn btn-outline-primary w-100" href="{{ route('web.evacuation-centers.index') }}">Evacuation Centers</a>
                    <a class="btn btn-outline-primary w-100" href="{{ route('web.relief-distributions.index') }}">Relief Distributions</a>
                    <a class="btn btn-outline-primary w-100" href="{{ route('web.damage-assessments.index') }}">Damage Assessments</a>
                    <a class="btn btn-outline-primary w-100" href="{{ route('web.notifications.index') }}">Notifications</a>
                    <a class="btn btn-outline-primary w-100" href="{{ route('web.response-team-members.index') }}">Response Team</a>
                    <a class="btn btn-outline-primary w-100" href="{{ route('web.calamity-reports.index') }}">Reports</a>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-2">
                    <a class="btn btn-sm btn-outline-light text-white" href="{{ route('calamities.index') }}">View All</a>
                </div>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @foreach($recentCalamities as $c)
                                <tr class="clickable-row" data-href="{{ route('calamities.show', $c) }}">
                                    <td class="fw-semibold">{{ $c->calamity_name ?? ucfirst($c->calamity_type) }}</td>
                                    <td>{{ ucfirst($c->calamity_type) }}</td>
                                    <td><small class="text-muted"><i class="bi bi-clock"></i> {{ optional($c->created_at)->diffForHumans() }}</small></td>

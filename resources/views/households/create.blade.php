...
$(document).ready(function() {
    ...
    function checkName(first, last, birthdate){
        first = (first||'').trim(); last = (last||'').trim(); birthdate = (birthdate||'').trim();
        if(!first || !last || !birthdate){ return Promise.resolve({exists:false}); }
        const url = dupCheckUrl + `?first_name=${encodeURIComponent(first)}&last_name=${encodeURIComponent(last)}&birthdate=${encodeURIComponent(birthdate)}`;
    ...
    const headBirth = document.getElementById('head_birthdate');
    function headHandler(){ checkName(headFirst.value, headLast.value, headBirth.value).then(res => {
        if(res && res.exists){
            hasDup = true; updateSubmitDisabled();
            renderError(headLast, `Resident '${headFirst.value.trim()} ${headLast.value.trim()}' with birthdate ${headBirth.value} already exists in the system`, res.url);
        } else {
            hasDup = false; updateSubmitDisabled(); clearError(headLast);
        }
    }); }
    if(headFirst && headLast && headBirth){ headFirst.addEventListener('input', debounce(headHandler, 400)); headLast.addEventListener('input', debounce(headHandler, 400)); headBirth.addEventListener('change', debounce(headHandler, 400)); }
    ...
    function attachMemberDupCheck(card){
        ...
        const birth = card.querySelector('input[name^="members"][name$="[birthdate]"]');
        if(!first || !last) return;
        const handler = debounce(function(){ checkName(first.value, last.value, birth ? birth.value : '').then(res => {
            if(res && res.exists){ hasDup = true; updateSubmitDisabled(); renderError(last, `Resident '${first.value.trim()} ${last.value.trim()}' with birthdate ${(birth?birth.value:'')} already exists in the system`, res.url); }
            else { hasDup = false; updateSubmitDisabled(); clearError(last); }
        }); }, 400);
        first.addEventListener('input', handler); last.addEventListener('input', handler); if(birth){ birth.addEventListener('change', handler); }
    }

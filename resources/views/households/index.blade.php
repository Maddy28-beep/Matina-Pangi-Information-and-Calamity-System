...
<div class="ds-page" data-search-scope>
    ...
    <div class="ds-page__header">
        ...
        <div class="ds-toolbar__group">
            <div class="ds-metric" role="presentation">
                ...
                <span class="ds-metric__value">{{ number_format($householdCount) }}</span>
            </div>
            @if(auth()->user()->isSecretary() && Route::has('households.create'))
                <a href="{{ route('households.create') }}" class="btn btn-primary d-none d-md-inline-flex">
                    <i class="bi bi-plus-circle"></i>
                    <span>Register Household</span>
                </a>
            @elseif(auth()->user()->isStaff() && Route::has('staff.households.create'))
                <a href="{{ route('staff.households.create') }}" class="btn btn-success d-none d-md-inline-flex">
    ...
    @if($households->count() > 0)
        <div class="ds-table-wrapper" id="householdsTableWrapper">
                        ...
                        @foreach($households as $household)
                            ...
                            >
                                ...
                                <td data-label="Primary Head">
                                    <div class="fw-semibold">{{ $headName }}</div>
                                </td>
                                <td data-label="Address" class="ds-text-truncate">{{ $household->address ?? '—' }}</td>
                                <td data-label="Purok">{{ $purokName }}</td>
                                ...
                                <td data-label="Status">
                                    @if($household->approval_status === 'active')
                                        <span class="ds-badge ds-badge--success">Active</span>
                                    @elseif($household->approval_status === 'inactive')
                                        <span class="ds-badge ds-badge--neutral">Inactive</span>
                                    @elseif($household->approval_status === 'pending')
                                        <span class="ds-badge ds-badge--warning">Pending</span>
                                    @else
                                        <span class="ds-badge ds-badge--neutral">{{ ucfirst($household->approval_status) }}</span>
                                    @endif
                                </td>
                                @if(auth()->user()->isSecretary())
                                    <td data-label="Actions" class="text-end" onclick="event.stopPropagation()">
                                        <div class="ds-actions">
                                            <a href="{{ route('households.edit', $household) }}" class="btn btn-icon btn-outline-secondary" title="Edit household">
                                            ...
                                            <button
                                                ...
                                                onclick="event.stopPropagation(); if(confirm('Archive this household? All residents will also be archived.')) { document.getElementById('archive-form-{{ $household->id }}').submit(); }">
                                                <i class="bi bi-archive"></i>
                                            </button>
                                        </div>
                                        <form id="archive-form-{{ $household->id }}" action="{{ route('households.archive', $household) }}" method="POST" class="d-none">
                                            @csrf
                                        </form>
                                    </td>
                                @endif
            ...
            <div class="ds-table-foot">
                @if(method_exists($households, 'firstItem') && $households->count() > 0)
                    <span class="ds-table-foot__meta">
                        Showing {{ number_format($households->firstItem()) }}–{{ number_format($households->lastItem()) }} of {{ number_format($householdCount) }} households
                    </span>
                @endif
                {{ $households->links() }}
            </div>
    ...
    @else
        <div class="ds-empty-state">
            ...
            @elseif(auth()->user()->isStaff() && Route::has('staff.households.create'))
                <a href="{{ route('staff.households.create') }}" class="btn btn-success mt-2">
                    <i class="bi bi-house-add"></i>
                    <span>Register first household</span>
                </a>
            @endif
    ...
    @if(auth()->user()->isSecretary() && Route::has('households.create'))
        <a href="{{ route('households.create') }}" class="fab d-md-none" aria-label="Register household">
            <i class="bi bi-plus-lg"></i>
        </a>
    @elseif(auth()->user()->isStaff() && Route::has('staff.households.create'))
        <a href="{{ route('staff.households.create') }}" class="fab d-md-none" aria-label="Register household">
            <i class="bi bi-plus-lg"></i>
        </a>
    @endif
        </div>

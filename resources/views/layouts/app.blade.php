...
<head>
    ...
    <title>@yield('title', 'Barangay Matina Pangi Information and Calamity System')</title>
...
<body>
    ...
    <div id="loading-screen"
        style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #ffffff; z-index: 9999; display: flex; align-items: center; justify-content: center; transition: opacity 0.3s ease;">
    ...
    <div class="min-h-screen bg-gray-50">
        ...
        <header
            style="position: fixed; top: 0; left: 0; right: 0; z-index: 1200; height: 64px; display: flex; align-items: center; justify-content: space-between; padding: 0 1.5rem; background: #ffffff; border-bottom: 1px solid #e5e7eb;">
            <!-- Left Section: Logo + System Name -->
            <div style="display: flex; align-items: center; gap: 0.75rem;">
                <!-- Logo & System Title -->
                <a href="{{ auth()->check() && auth()->user()->isCalamityHead() ? route('calamities.dashboard') : route('dashboard') }}"
            ...
            <div style="display: flex; align-items: center; gap: 1rem;">
                ...
                <button class="btn d-none d-md-flex align-items-center gap-2" data-bs-toggle="modal"
                    data-bs-target="#calendarModal"
                    style="background: #ffffff; border: 1px solid #e5e7eb; color: #374151; border-radius: 8px; padding: 0.5rem 1rem; font-size: 0.85rem; font-weight: 600; box-shadow: none;">
                ...
                @auth
                    ...
                    @if(auth()->user()->isSecretary())
                        ...
                        <div class="dropdown">
                            <button class="btn position-relative" type="button" data-bs-toggle="dropdown"
                                style="background: white; border: 1px solid #e5e7eb; border-radius: 8px; padding: 0.5rem; color: #6b7280;">
                                <i class="bi bi-bell" style="font-size: 1.1rem;"></i>
                                @if($annCount > 0)
                                    <span class="position-absolute badge rounded-pill bg-danger"
                                        style="top: -2px; right: -2px; font-size: 0.6rem; padding: 0.2em 0.4em;">
                            ...
                            <div class="dropdown-menu dropdown-menu-end shadow" style="min-width: 300px; border-radius: 12px;">
                                <div class="dropdown-header d-flex justify-content-between align-items-center">
                                    ...
                                    @if($annCount > 0)
                                        <span class="badge" style="background: var(--color-primary);">{{ $annCount }} new</span>
                                    @endif
                                </div>
                                @forelse($latestAnnouncements as $a)
                                    <a class="dropdown-item py-2" href="{{ route('announcements.show', $a) }}">
                                        <div class="d-flex">
                                            <i class="bi bi-megaphone me-2 mt-1" style="color: var(--color-primary);"></i>
                                            <div>
                                                <div style="font-size: 0.85rem; font-weight: 500;">
                                                    {{ \Illuminate\Support\Str::limit($a->title, 35) }}
                                                </div>
                    ...
                    <div class="dropdown">
                        <button class="btn d-flex align-items-center gap-2" type="button" data-bs-toggle="dropdown"
                            style="background: white; border: 1px solid #e5e7eb; border-radius: 8px; padding: 0.4rem 0.6rem;">
                            ...
                            <div class="d-none d-sm-block text-start" style="line-height: 1.1;">
                                <div style="font-size: 0.8rem; font-weight: 600; color: #374151;">{{ auth()->user()->name }}
                                </div>
                        ...
                        <div class="calendar-wrapper">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <button class="btn btn-sm" id="prevMonth"
                                    style="background: #f3f4f6; border: none; width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                            ...
                            <div style="display: grid; grid-template-columns: repeat(7, 1fr); gap: 8px;">
                                <!-- Day Headers -->
                                <div
                                    style="text-align: center; font-weight: 600; font-size: 0.75rem; color: #6b7280; padding: 8px 0;">
                                    Sun</div>
                                <div
                                    style="text-align: center; font-weight: 600; font-size: 0.75rem; color: #6b7280; padding: 8px 0;">
                                    Mon</div>
                                ...
                                <div id="calendarDays"
                                    style="grid-column: 1 / -1; display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px;">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="flex" style="padding-top: 64px;">
            ...
            <aside class="sidebar">
                <nav class="nav flex-column">
                    <!-- Dashboard -->
                    <a class="nav-link {{ (auth()->check() && auth()->user()->isCalamityHead() ? (request()->routeIs('calamities.dashboard') ? 'active' : '') : (request()->routeIs('dashboard') ? 'active' : '')) }}"
                        href="{{ auth()->check() && auth()->user()->isCalamityHead() ? route('calamities.dashboard') : route('dashboard') }}"
                    ...
                    @if(auth()->user()->isSecretary())
                        ...
                        <a class="nav-link" data-sidebar-toggle="submenu" data-target="#profilingSubmenu"
                            href="#profilingSubmenu" role="button"
                            aria-expanded="{{ request()->routeIs('residents.*', 'households.*', 'census.*', 'puroks.*', 'resident-transfers.*', 'certificates.*', 'approvals.*', 'archives.*') ? 'true' : 'false' }}"
                            aria-controls="profilingSubmenu" data-tooltip="Resident Profiling">
                            <i class="bi bi-person-vcard"></i>
                            <span class="nav-link-text">Resident Profiling</span>
                            <i class="bi bi-chevron-down ms-auto"></i>
                        </a>
                        <div class="collapse {{ request()->routeIs('residents.*', 'staff.residents.*', 'households.*', 'staff.households.*', 'census.*', 'puroks.*', 'resident-transfers.*', 'staff.resident-transfers.*', 'certificates.*', 'approvals.*', 'archives.*') ? 'show' : '' }}"
                            id="profilingSubmenu">
                            <nav class="nav flex-column ms-3">
                                <a class="nav-link {{ request()->routeIs('residents.*', 'staff.residents.*') ? 'active' : '' }}"
                                    href="{{ auth()->user()->isSecretary() ? route('residents.index') : route('staff.residents.index') }}"
                                    data-tooltip="Residents">
                                    <i class="bi bi-people"></i>
                                    <span class="nav-link-text">Residents</span>
                                </a>
                                <a class="nav-link {{ request()->routeIs('households.*', 'staff.households.*') ? 'active' : '' }}"
                                    href="{{ auth()->user()->isSecretary() ? route('households.index') : route('staff.households.index') }}"
                                    data-tooltip="Households">
                                    <i class="bi bi-house"></i>
                                    <span class="nav-link-text">Households</span>
                                </a>
                                <a class="nav-link {{ request()->routeIs('puroks.*') ? 'active' : '' }}"
                                    href="{{ route('puroks.index') }}" data-tooltip="Puroks">
                                    <i class="bi bi-geo-alt"></i>
                                    <span class="nav-link-text">Puroks</span>
                                </a>
                                <a class="nav-link {{ request()->routeIs('census.*') ? 'active' : '' }}"
                                    href="{{ route('census.index') }}" data-tooltip="Census">
                                    <i class="bi bi-bar-chart"></i>
                                    <span class="nav-link-text">Census</span>
                                </a>
                                <a class="nav-link {{ request()->routeIs('resident-transfers.*', 'staff.resident-transfers.*') ? 'active' : '' }}"
                                    href="{{ auth()->user()->isSecretary() ? route('resident-transfers.index') : route('staff.resident-transfers.index') }}"
                                ...
                                <a class="nav-link {{ request()->routeIs('certificates.*') ? 'active' : '' }}"
                                    href="{{ route('certificates.index') }}" data-tooltip="Certificates">
                                    <i class="bi bi-file-earmark-text"></i>
                                    <span class="nav-link-text">Certificates</span>
                                </a>
                                <a class="nav-link {{ request()->routeIs('approvals.*') ? 'active' : '' }}"
                                    href="{{ route('approvals.index') }}" data-tooltip="Approvals">
                        ...
                        @if(config('app.calamity_secretary_enabled'))
                            <a class="nav-link" data-sidebar-toggle="submenu" data-target="#calamitySubmenu"
                                href="#calamitySubmenu" role="button"
                                aria-expanded="{{ request()->routeIs('calamities.*', 'web.calamity-*', 'web.evacuation-*', 'web.relief-*', 'web.damage-*', 'web.notifications.*', 'notifications.*', 'web.response-*', 'response-team-members.*', 'web.calamity-reports.*') ? 'true' : 'false' }}"
                                aria-controls="calamitySubmenu" data-tooltip="Calamity Management">
                                <i class="bi bi-exclamation-triangle"></i>
                                <span class="nav-link-text">Calamity Management</span>
                                <i class="bi bi-chevron-down ms-auto"></i>
                            </a>
                            <div class="collapse {{ request()->routeIs('calamities.*', 'web.calamity-*', 'web.evacuation-*', 'web.relief-*', 'web.damage-*', 'web.notifications.*', 'notifications.*', 'web.response-*', 'response-team-members.*', 'web.calamity-reports.*') ? 'show' : '' }}"
                                id="calamitySubmenu">
                                <nav class="nav flex-column ms-3">
                                    <a class="nav-link {{ (request()->routeIs('calamities.*') && !request()->routeIs('calamities.dashboard')) ? 'active' : '' }}"
                                        href="{{ route('calamities.index') }}" data-tooltip="Calamity Incidents">
                                        <i class="bi bi-lightning"></i>
                                        <span class="nav-link-text">Calamity Incidents</span>
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('web.calamity-affected-households.*') ? 'active' : '' }}"
                                        href="{{ route('web.calamity-affected-households.index') }}"
                                        data-tooltip="Affected Households">
                                        <i class="bi bi-people"></i>
                                        <span class="nav-link-text">Affected Households</span>
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('web.evacuation-centers.*') ? 'active' : '' }}"
                                        href="{{ route('web.evacuation-centers.index') }}" data-tooltip="Evacuation Centers">
                                        <i class="bi bi-building"></i>
                                        <span class="nav-link-text">Evacuation Centers</span>
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('web.relief-items.*') ? 'active' : '' }}"
                                        href="{{ route('web.relief-items.index') }}" data-tooltip="Relief Goods">
                                        <i class="bi bi-box-seam"></i>
                                        <span class="nav-link-text">Relief Goods</span>
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('web.relief-distributions.*') ? 'active' : '' }}"
                                        href="{{ route('web.relief-distributions.index') }}" data-tooltip="Relief Distribution">
                                        <i class="bi bi-truck"></i>
                                        <span class="nav-link-text">Relief Distribution</span>
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('web.damage-assessments.*') ? 'active' : '' }}"
                                        href="{{ route('web.damage-assessments.index') }}" data-tooltip="Damage Assessment">
                                        <i class="bi bi-clipboard-check"></i>
                                        <span class="nav-link-text">Damage Assessment</span>
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('web.calamity-reports.*') ? 'active' : '' }}"
                                        href="{{ route('web.calamity-reports.index') }}" data-tooltip="Reports">
                                        <i class="bi bi-file-earmark-bar-graph"></i>
                                        <span class="nav-link-text">Reports</span>
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('web.notifications.*') ? 'active' : '' }}"
                                        href="{{ route('web.notifications.index') }}" data-tooltip="Emergency Notifications">
                                        <i class="bi bi-megaphone"></i>
                                        <span class="nav-link-text">Emergency Notifications</span>
                                    </a>
                                    <a class="nav-link {{ request()->routeIs('web.response-team-members.*') ? 'active' : '' }}"
                                        href="{{ route('web.response-team-members.index') }}" data-tooltip="Response Team">
                        ...
                        @endif
                    @elseif(auth()->user()->isCalamityHead())
                        <a class="nav-link {{ (request()->routeIs('calamities.*') && !request()->routeIs('calamities.dashboard')) ? 'active' : '' }}"
                            href="{{ route('calamities.index') }}" data-tooltip="Calamity Incidents">
                            <i class="bi bi-lightning"></i>
                            <span class="nav-link-text">Calamity Incidents</span>
                        ...
                        <a class="nav-link {{ request()->routeIs('web.calamity-affected-households.*') ? 'active' : '' }}"
                            href="{{ route('web.calamity-affected-households.index') }}" data-tooltip="Affected Households">
                            <i class="bi bi-people"></i>
                            <span class="nav-link-text">Affected Households</span>
                        </a>
                        <a class="nav-link {{ request()->routeIs('web.evacuation-centers.*') ? 'active' : '' }}"
                    ...
                    @if(auth()->user()->isStaff())
                        <a class="nav-link {{ request()->routeIs('staff.residents.index') ? 'active' : '' }}"
                            href="{{ route('staff.residents.index') }}" data-tooltip="Residents">
                            <i class="bi bi-people"></i>
                            <span class="nav-link-text">Residents</span>
                        </a>
                        <a class="nav-link {{ request()->routeIs('staff.households.*') ? 'active' : '' }}"
                            href="{{ route('staff.households.index') }}" data-tooltip="Households">
                            <i class="bi bi-house"></i>
                            <span class="nav-link-text">Households</span>
                        </a>
                        <a class="nav-link {{ request()->routeIs('staff.census.index') ? 'active' : '' }}"
                            href="{{ route('staff.census.index') }}" data-tooltip="Census">
                            <i class="bi bi-bar-chart"></i>
                            <span class="nav-link-text">Census</span>
                        </a>
                        <a class="nav-link {{ request()->routeIs('staff.puroks.index') ? 'active' : '' }}"
                            href="{{ route('staff.puroks.index') }}" data-tooltip="Puroks">
                            <i class="bi bi-geo-alt"></i>
                            <span class="nav-link-text">Puroks</span>
                        </a>
                        <a class="nav-link {{ request()->routeIs('staff.submissions.index') ? 'active' : '' }}"
                            href="{{ route('staff.submissions.index') }}" data-tooltip="My Submissions">
                            <i class="bi bi-file-earmark-text"></i>
                        ...
                        <a class="nav-link {{ request()->routeIs('staff.resident-transfers.*') ? 'active' : '' }}"
                            href="{{ route('staff.resident-transfers.index') }}" data-tooltip="Resident Transfers">
                            <i class="bi bi-arrow-left-right"></i>
                            <span class="nav-link-text">Resident Transfers</span>
                        </a>
                    ...
                    @if(auth()->user()->isSecretary())
                        ...
                        <a class="nav-link {{ request()->routeIs('announcements.*') ? 'active' : '' }}"
                            href="{{ route('announcements.index') }}" data-tooltip="Announcements">
                            <i class="bi bi-megaphone"></i>
                        ...
                        <a class="nav-link {{ request()->routeIs('archives.*') ? 'active' : '' }}"
                            href="{{ route('archives.index') }}" data-tooltip="Archives">
            ...
            <main class="main-content">
                @if(session('error'))
                    <div class="alert alert-danger alert-dismissible fade show d-flex align-items-center" role="alert"
                        style="margin-left: auto; max-width: fit-content;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('[data-sidebar-toggle="submenu"]').forEach(function (btn) {
